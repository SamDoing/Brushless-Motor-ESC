...
{
    if(TXSTAbits.BRGH)
        SPBRG = (48000000/16/rate)-1;
    else
        SPBRG = (48000000/64/rate)-1;
}
...
void buck_start()
{
    ...
    if(_buck_duty > 524 ) _buck_duty = 524; else if(_buck_duty < 0) _buck_duty = 0;
    CCPR2L = _buck_duty >> 2; CCP2CONbits.DC2B = 0x03 & _buck_duty;
}
...
void buck_continuos_controller()
{
    ...
    if(_buck_duty > 1023 ) _buck_duty = 1023; else if(_buck_duty < 0) _buck_duty = 0;
...
void __attribute__((picinterrupt(("")))) _int (void)
{
...
void trapzoidal_control()
{
    ...
    if(TMR0 > (hall_dt << 2)) trap_duty = 253;

    if(trap_duty > 253 ) trap_duty = 253; else if(trap_duty < 2) trap_duty = 2;
